...
public class ShouldHaveSameTime extends BasicErrorMessageFactory {
    ...
    private ShouldHaveSameTime(Date actual, Date expected) {
        super("%nExpecting%n <%s>%nto represent same time as:%n <%s>%nbut the time was different.%nActual time is%n "
              + "<%s>%nbut expected was:%n <%s>", actual, expected, actual.getTime(), expected.getTime());
    }
}
